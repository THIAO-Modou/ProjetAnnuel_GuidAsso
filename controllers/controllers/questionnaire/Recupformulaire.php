...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // Type de questionnaire rempli. La colonne TYPQUESTIONNARE est toujours renseignée automatiquement
        $TypeQuestionnaire = '';
       ...
       //Gestion du champs commune, on récupère la valeur de la commune ou de la case cochée
        ...
        if($reseau){
            ...
            if (!$row_structure) {
                // Insérer une nouvelle structure si la structure renseigée n'existe pas dans la base
        ...
        // Le champ Ressource est un tableau (array) et doit être converti en chaîne de caractères (string)
        ...
        // Mettre automatuement le nombre de rendez-vous à 1 pour les questionnaires dans la condition
        if($QR || $anonyme || $rechercheform || $reseau || $evenementform){
            $occurence = 1;
        }

        if($QR){
            // Mettre le temps à 10 automatiquemet pour Q/R
            $temps = 10; 
        }

        // Execution de l'insertion dans la table questionnaire
        $stmt->execute([
        ...
        // Redirection sur la meme page pour afficher le message
        header("Location: /../views/questionnaire.php");
        exit();
    } catch (Exception $e) {
        //  Message d'erreur s'il existe et toujours sur la meme page
