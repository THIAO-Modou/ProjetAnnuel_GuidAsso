...
document.addEventListener("DOMContentLoaded", function () {
  const assoField = document.getElementById("assoFieldLS");
  const assoInput = document.getElementById("association");
  const radios = document.querySelectorAll("input[name='choixAsso']");
// Si des éléments sont manquants, on sort du script
  if (!assoField || !assoInput || radios.length === 0) return;

  // Met à jour l'état du champ en fonction des radios
  function updateAssociationField() {
    const selected = [...radios].find(r => r.checked)?.value;

    // Cas où l’utilisateur choisit "projet" ou "non communiqué" → champ masqué
    if (selected === "projet" || selected === "noncommunique") {
      assoField.style.display = "none";
      assoInput.disabled = true;
      assoInput.value = "";
    } else {
      assoField.style.display = "flex";
      assoInput.disabled = false;
    }
  }

  // Ajoute un écouteur sur chaque bouton radio pour réagir au changement
  radios.forEach(radio => {
    radio.addEventListener("change", updateAssociationField);
  });

  // Si l'utilisateur clique dans l'input texte, désélectionner les radios
  assoInput.addEventListener("focus", function () {
    radios.forEach(r => r.checked = false);
    assoField.style.display = "flex";
    assoInput.disabled = false;
  });

  updateAssociationField(); // init
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (ficheAssocBtn) {
    ficheAssocBtn.addEventListener("click", function () {
      const url = "https://www.craig-guidasso.fr/../views/fiche_asso.php";
  ...
  rechercheRadios.forEach(radio => {
    radio.addEventListener("change", function () {
      if (this.checked && this.value === "Oui") {
        window.open(
          "https://www.craig-guidasso.fr/../views/questionnaire.php?formulaire=recherche", //  ton URL vers le formulaire
