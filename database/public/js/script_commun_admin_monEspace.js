                ...
                if (response.entries.length > 0) {
                    ...
                    response.entries.forEach(entry => {
                        ...
                        <tr>
                            ...
                            <td>${entry.NOMASSO ? entry.NOMASSO : ""}
                                ${entry.NOMSTRUCTURE ? entry.NOMSTRUCTURE : ""}
                                ${entry.NOMEVENEMENT ? entry.NOMEVENEMENT : ""}</td>
                            <td>${entry.CIVILITE ? entry.CIVILITE : ""} ${entry.NOMCONTACT ? entry.NOMCONTACT : ""}</td>
                            <td>${entry.THEMEGENERAL ? entry.THEMEGENERAL : ""}
                                ${entry.AUTRETHEMATIQUE ? entry.AUTRETHEMATIQUE : ""}</td>
                            <td>${entry.NOMUTILISATEUR ? entry.NOMUTILISATEUR : ""}
                                ${entry.PRENOMUTILISATEUR ? entry.PRENOMUTILISATEUR : ""}</td>
                        </tr>`); 
                    });
                } else {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (exportButton) {
        exportButton.addEventListener("click", function () {
            ...
            //  Construction dynamique de l’URL (GET)
