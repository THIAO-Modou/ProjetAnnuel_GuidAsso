        ...
        form.addEventListener("submit", function (e) {
            ...
            if (assocName) {
                fetch("/../controllers/predict_PHP/recherche_assoc.php", {
                    ...
                    body: `assoc=${encodeURIComponent(assocName)}`
                })
                .then(response => response.json())
                .then(data => {
                    resultsBody.innerHTML = data.length > 0 ? data.map(item => `
                        <tr>
            ...
            if (contactName) {
                fetch("/../controllers/predict_PHP/recherche_contact.php", {
                    method: "POST",
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    body: `nom_contact=${encodeURIComponent(contactName)}`
                })
                .then(response => response.json())
                .then(data => {
                    resultsBodyNom.innerHTML = data.length > 0 ? data.map(item => `
                        <tr>
                            <td>${item.HORODATEUR}</td>
                            <td>${item.THEMEGENERAL}</td>
                            <td>${item.AUTRETHEMATIQUE}</td>
